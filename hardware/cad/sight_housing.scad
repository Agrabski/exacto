...
// Hood underside dropped to just below the frame's top so the ceiling meets
// (and fuses to) the lens holder — small negative overlap for a solid joint.
...
module lens_frame() {
  ...
  difference() {
    ...
    // (see glass_frame_mount) so they subtract the exact same shapes and can
    // never fill the pocket or block the window.
    lens_negatives();
  }
}
...
module lens_negatives() {
  inner_hc = sqrt(lens_top_r * lens_top_r - (lens_w / 2) * (lens_w / 2));
  p_lcx = wall + lens_top_r;
  p_rcx = wall + lens_h - lens_top_r;
  p_cz = wall + lens_w / 2;
  p_r = lens_top_r + clearance / 2;
...
// shroud side wall to the frame's rounded ends.  The webs are kept to a
// central Z band (web_hz above/below the glass centreline) so, unlike a
// full-height arm, their corners never poke past the frame's tilted
// silhouette into the clear window opening.  They taper (hull) from the
